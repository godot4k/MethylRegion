# This file is derived from the covariate-adjusted DMR branch.
# Source branch: dmr.with.cov.lm
# Source files: package-r-naive/R/utils.R and package-r-naive/R/AMR.finder.R
.dmr_longitudinal_impl <- local({
...
buildBinaryMethModelData <- function(meth, y, cov.mod = NULL, NR = 1) {
  y_df <- as.data.frame(y)
  phenotype <- as.numeric(y_df[, 1])
  if (!all(is.finite(phenotype)) || !all(phenotype %in% c(0, 1))) {
    stop("For binary longitudinal mode, the first column of y must be coded as 0 and 1.")
  }
  if (length(unique(phenotype)) < 2) {
    stop("Both phenotype groups 0 and 1 are required.")
  }
...
selectRandomEffect <- function(model_dat) {
  candidates <- c(
    "family", "family_id", "Family", "FamilyID", "family.ID",
    "pair", "pair_id", "Pair", "PairID", "pair.ID",
...
extractPhenotypeCoefficient <- function(fit) {
  coef_table <- tryCatch(summary(fit)$coef, error = function(e) NULL)
  if (is.null(coef_table) || !"phenotype" %in% rownames(coef_table)) {
    return(c(p_value = 1, coef_lmm = 0))
  }
  p_col <- grep("^Pr\\(", colnames(coef_table), value = TRUE)
  if (length(p_col) == 0) {
    return(c(p_value = 1, coef_lmm = unname(coef_table["phenotype", 1])))
  }
  p_value <- unname(coef_table["phenotype", p_col[1]])
  coef_lmm <- unname(coef_table["phenotype", 1])
  if (!is.finite(p_value)) p_value <- 1
  if (!is.finite(coef_lmm)) coef_lmm <- 0
  c(p_value = p_value, coef_lmm = coef_lmm)
}

fitBinaryMethModel <- function(meth, y, cov.mod = NULL, NR = 1, method = "pearson") {
  model_dat <- buildBinaryMethModelData(meth, y, cov.mod, NR)
  model_dat <- model_dat[complete.cases(model_dat), , drop = FALSE]
  if (nrow(model_dat) < 3 ||
      length(unique(model_dat$phenotype)) < 2 ||
      length(unique(model_dat$meth)) < 2) {
    return(c(p_value = 1, coef_lmm = 0, cor_est = 0))
  }

  random_effect <- selectRandomEffect(model_dat)
  fixed_terms <- setdiff(names(model_dat), c("meth", "phenotype", random_effect))
  rhs <- c("phenotype", fixed_terms)
  fixed_formula <- as.formula(paste("meth ~", paste(rhs, collapse = " + ")))

  fit <- NULL
  if (!is.null(random_effect) && requireNamespace("lmerTest", quietly = TRUE)) {
    mixed_formula <- as.formula(
      paste("meth ~", paste(rhs, collapse = " + "), "+ (1 |", random_effect, ")")
    )
    model_dat[[random_effect]] <- factor(model_dat[[random_effect]])
    fit <- tryCatch(
      suppressMessages(suppressWarnings(lmerTest::lmer(mixed_formula, data = model_dat, REML = FALSE))),
  ...
  phenotype_coef <- extractPhenotypeCoefficient(fit)
  cor_est <- cor(model_dat$phenotype, model_dat$meth, use = "complete.obs", method = method)
  if (!is.finite(cor_est)) cor_est <- 0
  p_value <- unname(phenotype_coef["p_value"])
  coef_lmm <- unname(phenotype_coef["coef_lmm"])
  if (!is.finite(p_value)) p_value <- 1
  if (!is.finite(coef_lmm)) coef_lmm <- 0
  c(p_value = p_value, coef_lmm = coef_lmm, cor_est = cor_est)
}
...
cortest <- function(intput_dat, y, method = "pearson", cov.mod = NULL, a, b) {
  ...
  model_stat <- fitBinaryMethModel(x, y, cov.mod, NR, method)
  model_stat[!is.finite(model_stat)] <- c(p_value = 1, coef_lmm = 0, cor_est = 0)[!is.finite(model_stat)]
  return(c(unname(model_stat["p_value"]), unname(model_stat["coef_lmm"]), unname(model_stat["cor_est"])))
}
  ...
  while(length(stacks)||(a!=-1)){
    if((a!=-1)&&(child<=2)){
      ...
      if((newp<KS[1]||(newp>1&&KS[1]>1))&&(b-a>=mincpgs)){
        ...
        if(child==0){
          ...
          if(ab[1]>1&&n<=m){
            if(m-n>=mincpgs){
              ...
              bre<-list(chr=chr,start=n,stop=m,p_value=ks1[1],coef_lmm=ks1[2],cor_est=ks1[3])
        ...
        if(child==1){
          ...
          if(n<=m){
            if(m-n>=mincpgs){
              ...
              bre<-list(chr=chr,start=n,stop=m,p_value=ks2[1],coef_lmm=ks2[2],cor_est=ks2[3])
              breaks<-rbind(breaks,data.frame(bre))
            }
          }
        }
        if(child==2){
          ...
          if(n<=m){
            if(m-n>=mincpgs){
              ...
              bre<-list(chr=chr,start=n,stop=m,p_value=ks3[1],coef_lmm=ks3[2],cor_est=ks3[3])
              breaks<-rbind(breaks,data.frame(bre))
            }
          }
        }
        ...
        if(child==0){
          bre<-list(chr=chr,start=a,stop=b,p_value=KS[1],coef_lmm=KS[2],cor_est=KS[3])
          breaks<-rbind(breaks,data.frame(bre))
        }
        a<--1
        ab[1]<--1
    ...
    for(i in 1:nbreaks){
      ...
      if(b$p_value>1){
        ...
        if(!is.null(tmp)){
          ...
          if(ks[1]<2){
            out<-data.frame(chr=chr,start=intput_dat$pos[tmp_start]-1,stop=intput_dat$pos[tmp_stop],q=-1,length=tmp_stop-tmp_start+1,cor_est = ks[3],coef_lmm=ks[2],p_value=ks[1])
        ...
        out<-data.frame(chr=chr,start=intput_dat$pos[b$start]-1,stop=intput_dat$pos[b$stop],q=-1,length=b$stop-b$start+1,cor_est =b$cor_est,coef_lmm=b$coef_lmm,p_value=b$p_value)
...
AMRfinder <- function(intput_dat, y, cov.mod = NULL, controlist = list(
  ...
  colnames(nfo$outputList)[-4] <- c("chr", "start", "end", "N.CpGs", "cor_est", "coef_lmm", "p_value", "methX", "methY")
